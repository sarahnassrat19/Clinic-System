...
namespace Clinic_System
{
    public partial class recheck : Form
    {
        ...
        TextBox name;
        Button submit;
        bool is_patientExist , is_recheckExist;
        public void sign_in(object sender,EventArgs a)
        {
            if (is_patientExist == true && is_recheckExist == true)
            {
                MessageBox.Show("the patient is already registered and has recheck of date {0}", pp.date1);
                is_recheckExist = false;
            }
            else if(is_patientExist == true && is_recheckExist == false)
            {
                MessageBox.Show("the patient is already registered but has no recheck !","Sorry");
            }
            else
            {
                MessageBox.Show("the patient is not registered !"  , "Sorry");
        ...
        public recheck()
        {
            ...
            is_recheckExist = pp.recheck_date;
